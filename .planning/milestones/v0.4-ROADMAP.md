...
- [x] **Phase 8: Service Alerts Ingestion** - The server fetches, models, and filters currently-active GTFS-RT service alerts from DASH/Swiftly on a dedicated background poll, independent of the existing prediction poll (completed 2026-09-04)
- [x] **Phase 9: Alerts Surfaced on Routes, Stops & Predictions** - Riders see active alerts embedded directly in route, stop, and prediction responses, with no new dedicated alerts endpoint (completed 2026-09-08)

## Phase Details

### Phase 8: Service Alerts Ingestion

**Goal**: The server continuously fetches and maintains an up-to-date, filtered set of currently-active GTFS-RT service alerts from the DASH/Swiftly API, ready to be queried by other layers once Phase 9 wires them into responses.
**Depends on**: Nothing (extends the existing `BusDataRepository`/poll infrastructure; independent of Phase 9 until alerts need to be surfaced to clients)
...
- [x] 08-01-PLAN.md — Ingest, normalize, and filter GTFS-RT service alerts (fetch, model, boot-triggered 5-min poll, active-window query)
- [x] 08-02-PLAN.md — Gap closure (G-05-1): fix service-alerts endpoint 404 + open-ended active-window bug (WR-01/02/03)
- [x] 08-03-PLAN.md — Gap closure (G-05-2): JSON.parse fallback for string-encoded response body + entity -> entities field rename to match live API
- [x] 08-04-PLAN.md — Gap closure (G-05-3): array-root body-shape guard fix (WR-01) + carried-forward review findings
- [x] 08-05-PLAN.md — Gap closure (G-05-4): request format=json from DASH gtfs-rt-alerts/v2 to fix protobuf-vs-JSON root cause
- [x] 08-06-PLAN.md — Gap closure (G-05-5): confirm the live response's real top-level body shape via a human-run diagnostic capture, then fix fetchFromDashApi()/fetchAlerts() to handle it (no {entities} envelope required)

### Phase 9: Alerts Surfaced on Routes, Stops & Predictions
...
**Depends on**: Phase 8 (alerts must be ingested, modeled, and filterable before they can be embedded in responses)
...
- [x] 09-01-PLAN.md — Route alerts (ALRT-05, ALRT-06): ServiceAlertSummary model, mapping fn, ServiceAlertRepository.getActiveAlertsForRoute, BusRouteService embedding + DI wiring

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 09-02-PLAN.md — Stop alerts (ALRT-07, ALRT-08): ServiceAlertRepository.getActiveAlertsForStop, StopService embedding on getStopsForRoute/getNearbyStops + DI wiring
