...
A rider can send their location to `GET /api/v1/predictions/nearby` and get live arrival predictions for every stop around them (nearest-first, with distance and active service alerts) from one live call to DASH/Swiftly `real-time/{agency}/predictions-near-location`. Phase 10 (standalone, post-v0.4) appears in the snapshot below for context only. It is not part of v0.5, and its phase directory is archived under `v0.4-phases/`.

## Milestone Summary

**Key Decisions:**

- Parse nearby query params strictly and never clamp them: blank, array/object, non-finite or out-of-range values return 400 (radius ≤ 1mi, number ≤ 10)
- Validate each upstream prediction element, and drop only the offending entry with one warn (11-03 gap closure, CR-01)
- Wrap upstream errors from `error.message` only, never the axios error object, which carries `DASH_API_KEY` (T-11-01)
- Reuse the `RoutePrediction`/`Destination` shapes through a shared `predictionMapping` module

**Issues Resolved:**

- CR-01: a null prediction element returned 500 for every nearby stop, and a scalar element was served as an empty `{}` arrival. Plan 11-03 fixed both.

**Issues Deferred:**

- NEAR-10 (SSE nearby stream), NEAR-11 (route filter)
- SEED-001, SEED-002 (dormant seeds acknowledged at close)

**Technical Debt Incurred:**

- WR-01: the entry guard doesn't type-check display fields or `agencyKey`
- WR-02: the axios instance has no timeout
- WR-03: dropping a whole entry can remove a stop's alerts
- IN-01..IN-04: minor cleanups and test gaps (see `v0.5-phases/11-nearby-stop-predictions/11-REVIEW.md`)

**Closeout:** override_closeout. The v0.5 milestone audit was skipped by user choice. The Phase 11 verification override was recorded because VERIFICATION.md (status: passed, UAT 3/3) was flagged stale only because REQUIREMENTS.md checkboxes changed after verification ran. 2 seeds were acknowledged.

---

## Roadmap snapshot at close
