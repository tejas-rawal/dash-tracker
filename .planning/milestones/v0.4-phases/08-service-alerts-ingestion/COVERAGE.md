...
**Endpoint under audit:** DASH/Swiftly GTFS-RT service-alerts feed (exact path not yet confirmed in `INTEGRATIONS.md` or public docs — see the flagged assumption in `08-01-PLAN.md` Task 1). Capability surface below is the standard GTFS-RT `Alert` message shape (per the `gtfs-realtime.proto` spec, which Swiftly's real-time feeds implement), since that spec — not any DASH-specific wrapper — defines what the endpoint can expose.
...
| `informed_entity[].trip_id` / `agency_id` / `route_type` / `direction_id` | OPT-OUT | ALRT-03 scopes affected-entity capture to route(s)/stop(s) only; no Phase 9 requirement (ALRT-05..09) consumes these. Addable later without an architecture change. |
...
| `alert.image` (media/image URL) | OPT-OUT | Not referenced by ALRT-01..04; no client rendering surface exists yet to consume it (Phase 9 embeds JSON fields, not media). |
