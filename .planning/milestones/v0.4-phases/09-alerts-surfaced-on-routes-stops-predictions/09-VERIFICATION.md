...
human_verification:
  ...
  - test: "Same as above, but for StopService.getStopsForRoute()/getNearbyStops() (which call ServiceAlertRepository.getActiveAlertsForStop per stop) racing against a concurrent ServiceAlertPollService.applyAlerts() swap."
    ...
    why_human: "Identical reasoning to the route-lookup item above; this is the stop-side instance of the same 09-02-PLAN.md must_have, also tagged 'verification: backstop', also with no concurrency-exercising test in the suite."
---

# Phase 9: Alerts Surfaced on Routes, Stops & Predictions Verification Report
...
Merged from ROADMAP.md Phase 9 Success Criteria + `must_haves.truths` in both `09-01-PLAN.md` and `09-02-PLAN.md` (roadmap SCs are the higher-level restatement; the granular plan truths below are what were actually tested against the codebase).
...
|----------|---------------|--------|---------------------|--------|
| Route response `alerts` | `route.alerts` | `ServiceAlertRepository` in-memory Map, populated by `ServiceAlertPollService`'s 5-min poll (Phase 8) | Yes — live query, not static | ✓ FLOWING |
...
No orphaned requirements — REQUIREMENTS.md's Phase 9 traceability table lists exactly ALRT-05 through ALRT-08, and both plans jointly claim all four. (Note: REQUIREMENTS.md's checkboxes and traceability "Pending" label are still stale as of this verification — that's a documentation-sync task, not a code gap; it is expected to be updated in the phase's docs-evolution step after verification.)
