...
- Phase 6 (client-facing HTTP surface, per 05-01's objective) can proceed against `ServiceAlertRepository.getActiveAlerts()` with confidence in the live shape.
- Suggested follow-up: confirm whether `deletedAt`/`deletedBy` need to be respected in `getActiveAlerts()` filtering before Phase 6 exposes alerts publicly.
